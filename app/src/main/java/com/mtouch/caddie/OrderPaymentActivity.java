...
import com.mtouch.caddie.databinding.ActivityOrderPaymentBinding;
...
//                        prnSendData("신용카드전표", ksnetresp.PrnTempleteData(OrderPaymentActivity.this, 0, ksnetresp.getString("vat")));
    ...
    private void sendSmsPayCheck(String trackId,Runnable callback){
        ...
        caddieAPIService.smsPayCheck(map).enqueue(new Callback<com.mtouch.caddie.network.model.Response>() {
            @Override
    ...
    private void sendSmsPayPush(String trxId){
        ...
        map.put("trackId",trackId);
        caddieAPIService.smsPayPush(map).enqueue(new Callback<com.mtouch.caddie.network.model.Response>() {
            @Override
            public void onResponse(Call<com.mtouch.caddie.network.model.Response> call, Response<com.mtouch.caddie.network.model.Response> response) {
                if (response.isSuccessful()) {
