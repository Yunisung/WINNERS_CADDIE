...
public class InsecureSHA1PRNGKeyDerivator {
                        ...
                        if (bArr2.length != 0) {
                            ...
                            if (i2 < bArr2.length) {
                                ...
                                while (true) {
                                    ...
                                    this.nextBIndex = 0;
                                    int length2 = 20 < bArr2.length - i2 ? 20 : bArr2.length - i2;
                                    if (length2 > 0) {
                                        System.arraycopy(this.nextBytes, 0, bArr2, i2, length2);
                                        i2 += length2;
    ...
    private static void updateHash(int[] iArr, byte[] bArr, int i, int i2) {
        ...
        for (int i9 = 0; i9 < i6; i9++) {
            iArr[i7] = ((bArr[i8] & 255) << 24) | ((bArr[i8 + 1] & 255) << Handshake.CLIENT_KEY_EXCHANGE) | ((bArr[i8 + 2] & 255) << 8) | (bArr[i8 + 3] & 255);
            i8 += 4;
            i7++;
            if (i7 >= 16) {
                computeHash(iArr);
        ...
        if (i10 != 0) {
            ...
            if (i10 != 1) {
                i11 |= (bArr[i8 + 1] & 255) << Handshake.CLIENT_KEY_EXCHANGE;
                if (i10 != 2) {
                    i11 |= (bArr[i8 + 2] & 255) << 8;
                }
            }
